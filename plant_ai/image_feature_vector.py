...
def make_train_and_test_sets():
    ...
    for (dirname, subdirs, filenames) in tf.gfile.Walk(FLOWERS_DIR):
        ...
        # The sub directories give us the image files for training.
            ...
            examples = list(
                zip(full_filenames, [label_class] * len(filenames)))
...
print('\nThe dataset has %d label classes: %s' %
      (NUM_CLASSES, CLASSES.values()))
...
batch_images = tf.map_fn(decode_and_resize_image,
                         encoded_images, dtype=tf.float32)
...
def create_model(features):
    ...
    layer = tf.layers.dense(
        inputs=features, units=NUM_CLASSES, activation=None)
    return layer
    ...
    for i in range(NUM_TRAIN_STEPS):
        ...
        if i % EVAL_EVERY == 0 or is_final_step:
            ...
            test_loss, test_accuracy, test_prediction, correct_predicate = sess.run(
                ...
                feed_dict={encoded_images: batch_images, labels: batch_labels})
            print('Test accuracy at step %s: %.2f%%' %
                  (i, (test_accuracy * 100)))

            # 학습이 끝났을 때 모델을 저장함, global_step 숫자를 변경하여 새 이름으로 저장 추천 -------------
            if i == 99:
                saver = tf.train.Saver()
                saver.save(sess, "my_test_model", global_step=102)
...
# 저장했던 모델 불러오는 코드
# saver = tf.train.import_meta_graph('my_test_model-101.meta')
# with tf.Session() as sess:
# new_saver = tf.train.import_meta_graph('my_test_model-101.meta')
